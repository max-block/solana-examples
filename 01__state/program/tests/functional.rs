...
impl Env {
    async fn new() -> Self {
        ...
        let tx = Transaction::new_signed_with_payer(
            &[CounterInstruction::update_settings(&admin.pubkey(), *&admin.pubkey().to_bytes(), 1, 2)],
            Some(&admin.pubkey()),
            &[&admin],
            ctx.last_blockhash,
        );
        ...
        let acc = ctx.banks_client.get_account(Settings::get_settings_pubkey()).await.unwrap().unwrap();
        ...
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&user.pubkey()), &[&user], ctx.last_blockhash);
        ctx.banks_client.process_transaction(tx).await.unwrap();
...
async fn test_inc() {
    ...
    let acc = env.ctx.banks_client.get_account(Counter::get_counter_pubkey(&env.user.pubkey())).await.unwrap().unwrap();
...
async fn test_update_settings() {
    ...
    let tx = Transaction::new_signed_with_payer(
        &[CounterInstruction::update_settings(&env.admin.pubkey(), *&env.admin.pubkey().to_bytes(), 11, 22)],
    ...
    let acc = env.ctx.banks_client.get_account(Settings::get_settings_pubkey()).await.unwrap().unwrap();
    let settings = Settings::try_from_slice(acc.data.as_slice()).unwrap();
